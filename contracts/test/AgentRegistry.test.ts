...
describe("AgentRegistry", function () {
  ...
  describe("Agent Registration", function () {
    it("Should register a new agent", async function () {
      ...
      await expect(
        agentRegistry.connect(agent1Owner).registerAgent(
          agent1Id,
          "TestAgent1",
    ...
    it("Should fail to register agent with existing ID", async function () {
      ...
      await agentRegistry.connect(agent1Owner).registerAgent(
        agent1Id,
        "TestAgent1",
        "Description",
        "https://test.com",
      ...
      await expect(
        agentRegistry.connect(agent2Owner).registerAgent(
          agent1Id,
          "TestAgent2",
          "Description",
          "https://test2.com",
  ...
  describe("Agent Management", function () {
    ...
    it("Should update agent information (owner only)", async function () {
      ...
      await expect(
        agentRegistry.connect(agent1Owner).updateAgent(
          agent1Id,
          "UpdatedAgent1",
          "Updated description",
          "https://updated.com",
    ...
    it("Should fail to update agent by non-owner", async function () {
      ...
      await expect(
        agentRegistry.connect(user).updateAgent(
          agent1Id,
          "Hacked",
          "",
          "",
    ...
    it("Should deactivate agent (owner only)", async function () {
      await expect(
        agentRegistry.connect(agent1Owner).deactivateAgent(agent1Id)
    ...
    it("Should activate agent (owner only)", async function () {
      ...
      // Then activate
      await expect(
        agentRegistry.connect(agent1Owner).activateAgent(agent1Id)
  ...
  describe("Transaction Recording", function () {
    ...
    it("Should record a transaction", async function () {
      ...
      await expect(
        agentRegistry.connect(user).recordTransaction(txId, agent1Id, amount)
    ...
    it("Should update transaction rating", async function () {
      ...
      // Update rating
      await expect(
        agentRegistry.connect(user).updateTransactionRating(txId, 5)
      ).to.emit(agentRegistry, "RatingUpdated");
    ...
    it("Should fail to update rating with invalid value", async function () {
      ...
      await expect(
        agentRegistry.connect(user).updateTransactionRating(txId, 0)
      ).to.be.revertedWith("AgentRegistry: rating must be 1-5");

      await expect(
        agentRegistry.connect(user).updateTransactionRating(txId, 6)
      ).to.be.revertedWith("AgentRegistry: rating must be 1-5");
    });
  });
  ...
  describe("Search and Discovery", function () {
    ...
    it("Should get total counts", async function () {
      ...
      const txId = ethers.id("tx1");
      await agentRegistry.connect(user).recordTransaction(
        txId,
        agent1Id,
